...
#_last_guess;
  ...
  loadRegionTiles() { // removed parameter as it is not used
  ...
  randomize(radius=50) {
    ...
    outerloop: while(!done) { 
      do {
        ...
        if(rando == undefined) {done = true; break outerloop;}
        rando = rando["add"];
...
function* poissonDiscSampler(width, height, radius) {
  ...
  function sample(x, y) { // removed parameter parent as it was never used
